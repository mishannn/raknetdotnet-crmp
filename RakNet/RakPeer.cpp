...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
		...
		if ((packet->length >= sizeof(unsigned char) + sizeof(RakNetTime)) &&
		...
		} else if ((unsigned char)packet->data[0] == ID_RPC_REPLY) {
			...
			packet = 0; // Will do the loop again and get another packet
		} else if ((unsigned char)packet->data[0] == ID_CONNECTION_REQUEST_ACCEPTED) {
				...
				char *auth_bs = "26CF68E9E2FF12368E424417120DC2F2F24AA96900E";
				...
				bsSend.Write(iVersion + 0x2000000);
...
bool RakPeer::HandleRPCPacket(const char *data, int length, PlayerID playerId) {
	...
	if (hasUserData) {
		if (usedAlloca == false && userData != nullptr)
			delete[] userData;
	}
...
#endif
{
	...
	fprintf(f, "RECV: %d \n%s\n", length, DumpMem((uint8_t *)data, length));
	fclose(f);*/
...
//	unsigned numberOfBytesUsed;
	...
	while (rcs) {
		...
		if (rcs->nextRequestTime < timeMS) {
			...
			*(WORD*)&c[1] = (WORD)rcs->playerId.port ^ 0x4196;
