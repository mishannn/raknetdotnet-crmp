...
void ReliabilityLayer::SendBitStream( SOCKET s, PlayerID playerId, RakNet::BitStream *bitStream )
{
	...
	if (data[0] == 0x00 && data[1] == 0x80 && data[2] == 0x07 && data[3] == 0xA0) {
		if (length > 6) {
			RakNet::BitStream bs;
			bs.Write((char*)"\x01\x00\x30\x00\x00\x11\x80\x20", 8);
			bs.Write((char*)data + 14, length - 14);
			bitStream->Reset();
			length -= 6;

			SocketLayer::Instance()->SendTo( s, ( char* ) bs.GetData(), length, playerId.binaryAddress, playerId.port );
		} else {
			bitStream->Reset();
		}
	} 
